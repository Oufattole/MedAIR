...
class TfidfDocRanker(object):
    ...
    def closest_docs(self, query, k=100):
    ...
    def batch_closest_docs(self, queries, k=100, num_workers=None):
    ...
    def solve_question(self, question):
        ...
        search = partial(self.closest_docs, k=self.topn)
        queries = []
        if self.tokenize:
        ...
        scores = []
        for _, doc_scores in map(search, queries):
            assert(len(doc_scores)==self.topn)
            scores.append(sum(doc_scores))
    ...
    def solve_dev_set(self):
        logger.info(f'Solving dev set with tfidf')
    ...
    def solve_test_set(self):
        logger.info(f'Solving test set with tfidf')
        question_filename = "/questions/test.jsonl"
        questions = Question.read_jsonl(DATA_DIR + question_filename)
        return self.solve_question_set(questions)
    def solve_train_set(self):
        logger.info(f'Solving training set with tfidf')
        question_filename = "/questions/train.jsonl"
        questions = Question.read_jsonl(DATA_DIR + question_filename)
        return self.solve_question_set(questions)
if __name__ == "__main__":
    tfidf_rank = TfidfDocRanker(tokenize=False, topn=10)
    print(tfidf_rank.solve_dev_set())
    print(tfidf_rank.solve_test_set())
    print(tfidf_rank.solve_train_set())